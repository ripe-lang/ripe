...
let%expect_test "null assigned to pointer" =
  run [ func "f" [ Let ("p", Some (Pointer (Named "i32")), Null) ] ];
  [%expect {| ok |}]

let%expect_test "break inside while" =
  run [ func "f" [ While (Bool true, [ Break ]) ] ];
  [%expect {| ok |}]

let%expect_test "forward reference" =
  run [ func "f" [ Expr (Call ("g", [])) ]; func "g" [] ];
  [%expect {| ok |}]
