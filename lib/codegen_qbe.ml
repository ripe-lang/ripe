...
type ctx = {
  ...
  filename : string;
  cur_func : string ref;
  strtab : string list ref;
  strtab_len : int ref;
  strtab_offs : (string, int) Hashtbl.t;
  sites : (int * int * int * int) list ref;
  site_ctr : int ref;
}
...
(* The strings end with a zero byte so the runtime doesn't need a length *)
let strtab_off (ctx : ctx) (s : string) : int =
  match Hashtbl.find_opt ctx.strtab_offs s with
  | Some off -> off
  | None ->
      let off = !(ctx.strtab_len) in
      ctx.strtab := s :: !(ctx.strtab);
      ctx.strtab_len := off + String.length s + 1;
      Hashtbl.replace ctx.strtab_offs s off;
      off

let panic_site (ctx : ctx) (span : Ast.span) : int =
  if span.Span.file < 0 then Error.ice "runtime check has no source location";
  let line, col = Source_map.lookup ctx.sm span.Span.lo in
  let entry =
    (strtab_off ctx ctx.filename, line, col, strtab_off ctx !(ctx.cur_func))
  in
  let id = !(ctx.site_ctr) in
  incr ctx.site_ctr;
  ctx.sites := entry :: !(ctx.sites);
  id
...
(* TODO(2cc1): the local arithmetic always emits instructions instead of folding
   through fold_const_num *)
...
let emit_panic_tables (ctx : ctx) =
  if !(ctx.sites) <> [] then begin
    let chunk s = Printf.sprintf "b \"%s\", b 0" s in
    emit ctx "export data $ripe_panic_strtab = { %s }\n"
      (String.concat ", " (List.map chunk (List.rev !(ctx.strtab))));
    let entry (file, line, col, func) =
      Printf.sprintf "w %d, w %d, w %d, w %d" file line col func
    in
    emit ctx "export data $ripe_panic_sites = align 4 { %s }\n"
      (String.concat ", " (List.map entry (List.rev !(ctx.sites))))
  end

let emit_qbe ~(sm : Source_map.t) ~(filename : string) (tdecls : T.cdecl list) :
    string =
